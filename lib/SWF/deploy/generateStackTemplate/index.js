...
function generateStackTemplate({
  ...
  if (codeSha256) {
    addLambdaFunction({
      name: 'CustomTypes',
      handler: 'lib/SWF/CFCustomTypes.default',
  ...
  if (tasks) {
    ...
    addLambdaFunction({
      name: 'decider',
      handler: 'lib/SWF/Decider.default',
      env: {
        WORKFLOWS_PATH,
        TASKS_PATH,
    ...
    addLambdaFunction({
      name: 'deciderSpawner',
      handler: 'lib/SWF/DeciderSpawner.default',
      env: {
        SOFLOW_WORKFLOWS_VERSION: version,
        SOFLOW_NAMESPACE: namespace,
        SOFLOW_DOMAIN: domain,
    ...
    for (const taskName of Object.keys(tasks)) {
      ...
      addLambdaFunction({
        name: `${taskName}`,
        handler: `lib/SWF/LambdaTaskHandlers.${taskName}`,
        timeout,
        description,
        env: {TASKS_PATH}
      })
